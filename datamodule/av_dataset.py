...
def load_video(path):
    ...
    """
    vid = torchvision.io.read_video(path, pts_unit="sec")[0]  # , output_format="THWC"
...
if __name__ == '__main__':
    ...
    root, label_path, subset, modality, audio_transform, video_transform = \
        '/45TB/xx/CMLRdataset', '../tools/train.csv', 'train', 'video', None, VideoTransform("train")
    dataset = AVDataset(root,
                        label_path,
                        subset,
                        modality,
                        audio_transform,
                        video_transform)
    print(dataset[6]['input'].shape)
